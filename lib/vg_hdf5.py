...
class vg_hdf5(Dataset):
    def __init__(self, cfg, split="train", transforms=None, num_im=-1, num_val_im=5000,
        ...
        self.split_mask, self.image_index, self.im_sizes, self.gt_boxes, self.gt_classes, self.relationships, self.predicates = load_graphs(
            ...
            filter_non_overlap=filter_non_overlap and 'train' in split,
        )
...
def load_graphs(graphs_file, images_file, mode='train', num_im=-1, num_val_im=0, filter_empty_rels=True,
    ...
    :param mode: (train, train_frequent, train_few_shot, val, val_frequent, val_few_shot, test, test_frequent, or test_few_shot)
    ...
    """
    if mode not in ('train', 'train_frequent', 'train_few_shot', 'val', 'val_frequent', 'val_few_shot', 'test', 'test_frequent', 'test_few_shot'):
    ...
    split = 2 if 'test' in mode else 0
    ...
    # count predicates per class and split the predicates into two sets
    relation_predicates = list(_relation_predicates)
    if 'frequent' in mode or 'few_shot' in mode:
        relation_predicates_count = {i:relation_predicates.count(i) for i in range(1, 51)}  # except the background
        relation_predicates_sorted = sorted(relation_predicates_count.items(), key=lambda item:item[1], reverse=True)
        relation_predicates = [relation_predicates_sorted[i][0] for i in range(25)] \
            if 'frequent' in mode else [relation_predicates_sorted[i][0] for i in range(25, 50)]
    ...
    for i in range(len(image_index)):
        ...
        if im_to_first_rel[i] >= 0:
            ...
            inds = np.array([predicate in relation_predicates for predicate in predicates])
        else:
        ...
        relationships.append(rels[inds, :])

    im_sizes = np.stack(im_sizes, 0)
    return split_mask, image_index_valid, im_sizes, boxes, gt_classes, relationships, relation_predicates
